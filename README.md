...
> 安定版1.0.2では、2台のスマートフォンによる同時接続と個別切断を実機確認済みです。アプリ内に台数制限はありませんが、3台以上の同時接続は未検証で、Bluetoothアダプターやドライバーに依存します。
...
タスクトレイを左クリックすると、最後に閉じた通常画面またはクイック操作画面が復帰します。通常画面、クイック操作画面、右クリックメニューの端末は名前の昇順（A→Z）で表示され、接続状態が変わっても位置は入れ替わりません。右クリックメニューには最大6台が表示され、端末ごとに接続・切断できます。7台以上見つかった場合は、**その他の端末…**から全端末を扱えるクイック操作画面を開けます。
